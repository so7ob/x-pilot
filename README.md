...
> **v1.13.2:** رابط المنشور المنشور صار صحيحًا حتمًا — بعد أن كان السجل يلتقط أحيانًا رابط تغريدة لغيرك أو رابط تحليلات بلا suffix، صار الرابط يؤخذ فقط من إشعار النشر الذي يعرضه X بعد النجاح، أو من روابط status الجديدة باسم حسابك التي ظهرت بعد نقرة النشر، أو من عنوان الصفحة إذا كان رابط منشورك — ويُقيَّس دائمًا إلى الصيغة القياسية بلا لاحقات تحليلات، وحارس شكل في المحرك يمنع أي رابط غير قياسي من الوصول إلى السجل، وما لا يمكن تأكيده يُعرض بصدق كـ«رابط غير متاح» بدل رابط خاطئ.

> **v1.13.1:** إصلاح جذري لسجل الجلسات — بعد أن كانت جلسة حقيقية (٥ نجاحات وفشلان) تظهر صفرًا في كل شيء، صار كل صف محاولة يُسجَّل ويظهر حتمًا في بطاقته: معرف السجل = معرف جلسة التشغيل بالبناء، العدادات والحالة تتحدث حيًّا بعد كل محاولة، البدايات المجدولة تنشئ سجلها، اللقطة التحليلية الكاملة (نص التغريدة، البنك، الموضع، المدة، ورابط المنشور للنجاح) تنجو من دورة التخزين، وتمريرة إصلاح محافظة تعيد ربط بياناتك القديمة الموجودة عند بدء الخلفية دون حذف أي شيء.
