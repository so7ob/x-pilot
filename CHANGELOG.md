# Changelog
## [1.13.2] - رابط المنشور المنشور يصبح صحيحًا حتمًا
### Fixed
- **رابط المنشور المسجَّل كان لتغريدة شخص آخر**: كان مستخرِج الرابط يأخذ آخر رابط `status` يوجد في الصفحة كلها بعد النشر — والصفحة بعد النشر مليئة بروابط تغريدات الآخرين وصفوف التحليلات، فسُجِّل مرة رابط مثل `x.com/حساب-آخر/status/123/analytics` بدل رابط منشورك الحقيقي، وكان نمط المطابقة لا يربط النهاية فتمرّ لاحقات `/analytics` كما هي. صار الاستخراج يثق فقط بإشارات ما بعد النشر بهذا الترتيب الصارم: **رابط إشعار النشر (toast)** الذي يعرضه X بنفسه بعد النجاح، ثم **روابط status الجديدة التي ظهرت بعد نقرة النشر فقط** (بمقارنتها بلقطة مجمدة تُؤخذ قبل النقر مباشرة) والمنسوبة إلى حسابك المسجَّل، ثم **عنوان الصفحة نفسه** إذا كان رابط منشور لحسابك (أو إعادة التوجيه الداخلية `/i/web/status/{id}` لمنشورك). ما عدا ذلك: لا رابط — ورسالة «رابط المنشور غير متاح» الصادقة أفضل دائمًا من رابط خاطئ.
- **تقييس الروابط وحارس الشكل**: كل رابط منشور يُسجَّل بصيغته القيانية `https://x.com/{الحساب}/status/{المعرف}` مع إزالة لاحقات `/analytics` و`/photo/1` واستعلامات الروابط وتجزئاتها، وحارس شكل خالص في المحرك (`sanitizePublishedPostUrl`) يمنع وصول أي شيء ليس رابط منشور قياسيًا إلى السجل أصلًا، وحارس حساب يرفض أي رابط ليس باسم الحساب المسجَّل (مطابقة غير حساسة لحالة الأحرف وتتسامح مع @).

### Safety
- لا تغيير على أي منطق نشر أو بوابة إعادة النشر المحظورة؛ المحاولات تبقى سجلًا غير قابل للتغيير؛ schemaVersion يبقى 4 ولم تُمس الهجرات.
- وحدة الاستخراج الجديدة (`src/domain/published-post-url.ts`) نقية بلا أي وصول إلى DOM أو chrome، والمحرك يتعامل مع مخرجاتها فقط.
- الاختبارات نمت من 338 إلى 352: تثبيت حالة المستخدم الحقيقية (صفحة مليئة بروابط الغرباء وصف تحليلات حساب آخر بينما يجب أن يفوز رابط إشعار النشر)، ورفض رابط إشعار لحساب مغاير، وقبول رابطك الجديد الظاهر بعد النقر فقط، ورفض الروابط القديمة وروابط الغرباء الجديدة معًا، وقيوع صفحة الرد على تغريدة غيرك، ومصفوفة التقييس الكاملة — مع عقود تمنع عودة استراتيجية «آخر رابط في الصفحة» إلى الأبد. وكلها ناجحة والبناء نقي.

## [1.13.1] - إصلاح تسجيل سجل الجلسات
### Fixed
- **كل محاولة تُسجَّل الآن فعلًا**: عولج الخلل الجذري الذي جعل جلسة حقيقية بخمس عمليات ناجحة وفشلتين تظهر صفرًا في كل شيء — كان معرف سجل الجلسة التحليلي يُولَّد جديدًا بينما كل صفوف المحاولات تُخزَّن بمفتاح معرف جلسة التشغيل، فلا يطابق أحدهما الآخر أبدًا؛ ومع أن البيانات كانت محفوظة ظلت غير مرئية. الآن **معرف السجل = معرف جلسة التشغيل** بالبناء، فكل صف محاولة (نجاحًا أو فشلًا) يجد بطاقته دائمًا مع تاريخه ووقته ونتيجته وكل بياناته.
- **عدادات الجلسة تتحدث حيًّا**: كان `syncHistoricalSession` يبحث عن معرف سجل لا وجود له بعد أي دورة تخزين (لأن `sessionFromRuntime` يصنعه من معرف جلسة التشغيل) فيتوقف صامتًا إلى الأبد — لذلك ظلت البطاقة «قيد التشغيل» بعدادات صفر ومدة «—». بمحاذاة المعرفات أصبح التحديث يصل: المنشور والفاشل والمتخطى والحالة ووقت الانتهاء.
- **البدايات المجدولة تنشئ سجلًا**: شرط `!historicalSessionId` كان يفشل دائمًا بعد أول دورة تخزين فلا يُنشأ سجل أبدًا للجلسات المجدولة — استُبدل بضمانة معرفية مناطقة `ensureHistoricalSession` تفحص الوجود بالمعرف نفسه.
- **اللقطة التحليلية تنجو من التخزين**: كانت طبقة v4 تُسقط كل الحقول التحليلية عند الحفظ والقراءة (رابط المنشور، نص التغريدة، البنك واسمه، الموضع، المدة، المهايئ) — زوج `toV4Attempt`/`fromV4Attempt` النقي يحفظها الآن جميعًا، ونتيجة «PAUSED» تُخزَّن كما هي بدل تحويلها صامتًا إلى «فاشل».

### Safety
- **إصلاح البيانات القديمة بلا أي إتلاف**: ممر إصلاح نقي ومحافظ (`repairSessionHistoryLinks`) يعمل عند بدء الخلفية عبر كل مساحات العمل ويعيد ربط السجلات القديمة بمحاولاتها: تبنّي السجل القشرة الفعّال الوحيد ذي المحاولات الصفرية بمعرف الجلسة الحية، وإقران المجموعات اليتيمة بسجل نهائي فريد تتضمن نافذته الزمنية محاولاته — أي غموض يُترك كما هو بصدق، ولا يُحذف أي شيء مطلقًا (إعادة تسمية معرف فقط)، وهو عاطل الإعادة فلا يتكرر أثره.
- لا مساس بأي سطح نشر؛ المحاولات تبقى سجلًا غير قابل للتغيير؛ schemaVersion يبقى 4 وسجل الهجرات لم يُلمس.
- الاختبارات نمت من 322 إلى 338 (مصفوفة المحاذاة والترميم الكاملة + عقود تمنع حرّاس `historicalSessionId` الحيثية وتثبت بقاء الحقول التحليلية في v4) وكلها ناجحة، والبناء نقي.
